  ...
  return (
    <div className="border border-gray-200 rounded-xl overflow-hidden">
      ...
      <div className="flex items-center gap-1 px-3 py-2 border-b border-gray-200 bg-gray-50 flex-wrap">
        <button type="button" onClick={() => execCommand("bold")} className="p-1.5 rounded hover:bg-gray-200 transition-colors" title="Bold">
          <BiBold size={16} className="text-gray-600" />
        </button>
        <button type="button" onClick={() => execCommand("italic")} className="p-1.5 rounded hover:bg-gray-200 transition-colors" title="Italic">
          <span className="text-gray-600 text-sm italic font-serif">I</span>
        </button>
        <button type="button" onClick={() => execCommand("underline")} className="p-1.5 rounded hover:bg-gray-200 transition-colors" title="Underline">
          <span className="text-gray-600 text-sm underline">U</span>
        </button>
        <div className="h-5 w-px bg-gray-300 mx-1" />
        <button type="button" onClick={() => execCommand("formatBlock", "h2")} className="p-1.5 rounded hover:bg-gray-200 transition-colors" title="Heading">
          <span className="text-gray-600 text-xs font-bold">H2</span>
        </button>
        <button type="button" onClick={() => execCommand("formatBlock", "h3")} className="p-1.5 rounded hover:bg-gray-200 transition-colors" title="Sub Heading">
          <span className="text-gray-600 text-xs font-bold">H3</span>
        </button>
        <div className="h-5 w-px bg-gray-300 mx-1" />
        <button type="button" onClick={() => execCommand("insertUnorderedList")} className="p-1.5 rounded hover:bg-gray-200 transition-colors" title="List">
          <BiListUl size={16} className="text-gray-600" />
        </button>
        <button type="button" onClick={() => execCommand("insertOrderedList")} className="p-1.5 rounded hover:bg-gray-200 transition-colors" title="Numbered List">
        ...
        <button
          ...
          className={`p-1.5 rounded transition-colors ${mode === "html" ? "bg-gray-200" : "hover:bg-gray-200"}`}
          title="HTML source"
        >
      ...
      {mode === "visual" ? (
        ...
        <textarea
          ...
          className="w-full min-h-[200px] px-4 py-3 text-sm text-gray-800 font-mono focus:outline-none resize-y"
    ...
    <div className="space-y-4">
      <div>
        ...
        <input
          ...
          className="w-full rounded-xl border border-gray-200 h-11 px-4 text-sm focus:outline-none focus:ring-1 focus:ring-[#F46A6A]/20 focus:border-[#F46A6A] transition-all placeholder:text-gray-300"
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          <select
            ...
            className="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-[#F46A6A]/20 focus:border-[#F46A6A] transition-all bg-white"
          >
            {CATEGORIES.map((cat) => (
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <input
            ...
            className="w-full rounded-xl border border-gray-200 h-11 px-4 text-sm focus:outline-none focus:ring-1 focus:ring-[#F46A6A]/20 focus:border-[#F46A6A] transition-all"
          />
        </div>
        <div>
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="flex gap-3">
            <div className="relative">
              ...
              <select
                ...
                className="pl-8 pr-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-1 focus:ring-[#F46A6A]/20 focus:border-[#F46A6A] transition-all bg-white appearance-none min-w-[150px]"
              >
                ...
                {paginated.map((article) => (
                  <tr key={article.id} className="hover:bg-gray-50/50 transition-colors">
                    <td className="px-6 py-4">
                      <p className="text-sm font-medium text-gray-900 truncate max-w-[250px]">{article.title}</p>
                    </td>
                    <td className="px-6 py-4 hidden md:table-cell">
                      <span className="text-sm text-gray-600">{article.author}</span>
                    </td>
                    <td className="px-6 py-4 hidden lg:table-cell">
                      <span className="inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium bg-rose-50 text-[#F46A6A]">
                        {article.category}
                      </span>
                    </td>
                    <td className="px-6 py-4 hidden md:table-cell">
      ...
      <Modal isOpen={showAddModal} onClose={() => setShowAddModal(false)} title="New Article" size="xl">
        <ArticleForm onSubmit={handleAdd} submitLabel="Create Article" />
      </Modal>

      {/* Edit Article Modal */}
      <Modal isOpen={showEditModal} onClose={() => setShowEditModal(false)} title="Edit Article" size="xl">
        <ArticleForm onSubmit={handleEdit} submitLabel="Save Changes" />
      </Modal>
